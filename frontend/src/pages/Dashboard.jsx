...
const STATUS_STYLES = {
  saved:      { bg: 'bg-gray-100',    text: 'text-gray-600',    dot: 'bg-gray-400',    label: 'Saved' },
  applied:    { bg: 'bg-blue-50',     text: 'text-blue-600',    dot: 'bg-blue-500',    label: 'Applied' },
  interview:  { bg: 'bg-amber-50',    text: 'text-amber-600',   dot: 'bg-amber-500',   label: 'Interview' },
    ...
    >
      ...
      {!resume && (
        <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-amber-700 font-medium">No resume uploaded — match scores won't be calculated</span>
          </div>
          <Link to="/jobs/new" className="text-xs font-semibold text-amber-600 hover:underline">
            Upload now →
          </Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: 'Offers', value: stats.offer, color: 'text-emerald-600', bg: 'bg-emerald-50' },
        ].map((stat, i) => (
          <div
            key={i}
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-gray-500 text-sm mb-4">
            {filter === 'all' ? 'Add your first job to get started' : `No jobs with status "${filter}"`}
          </p>
          <Link
            to="/jobs/new"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <span className="text-gray-500 text-xs">{job.company || 'Unknown company'}</span>
                    {job.location && (
                      <>
                    ...
                    {job.location_type && (
                      <>
                        <span className="text-gray-300">·</span>
                        <span className="text-gray-500 text-xs capitalize">{job.location_type}</span>
                      </>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-4 flex-shrink-0">
                  <span className={`hidden sm:inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${s.bg} ${s.text}`}>
