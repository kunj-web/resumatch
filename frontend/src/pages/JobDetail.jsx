...
const STATUS_OPTIONS = [
  ...
  { value: 'applied',   label: 'Applied',   color: 'bg-blue-50 text-blue-600 border-blue-200' },
  { value: 'interview', label: 'Interview', color: 'bg-amber-50 text-amber-600 border-amber-200' },
  { value: 'offer',     label: 'Offer',     color: 'bg-emerald-50 text-emerald-600 border-emerald-200' },
...
const CATEGORY_STYLES = {
  ...
  tool:         { bg: 'bg-violet-50',  text: 'text-violet-700',  border: 'border-violet-200' },
  soft:         { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200' },
  certification:{ bg: 'bg-amber-50',   text: 'text-amber-700',   border: 'border-amber-200' },
...
export default function JobDetail() {
  ...
  if (!job) return (
    <div className="flex flex-col items-center justify-center py-20">
      ...
      <button onClick={() => navigate('/')} className="mt-4 text-emerald-600 text-sm hover:underline">
        Back to dashboard
      </button>
    </div>
  )
    ...
    >
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-start gap-4">
            <div
              ...
              style={{ background: 'linear-gradient(135deg, #0d1b2a, #112240)' }}
            >
              {job.company ? job.company[0].toUpperCase() : '?'}
            </div>
            <div>
              <h1 className="text-xl font-bold text-gray-900">{job.title || 'Untitled Role'}</h1>
              <div className="flex items-center gap-2 mt-1 flex-wrap">
                <span className="text-gray-600 text-sm font-medium">{job.company || 'Unknown company'}</span>
                {job.location && (
                  <>
                ...
                {job.location_type && (
                  <>
                    <span className="text-gray-300">·</span>
                    <span className="text-gray-500 text-sm capitalize">{job.location_type}</span>
                  </>
                )}
                {job.job_type && (
                  <>
                    <span className="text-gray-300">·</span>
                    <span className="text-gray-500 text-sm capitalize">{job.job_type.replace('-', ' ')}</span>
                  </>
                )}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            {job.applied_at && (
              <p className="text-xs text-gray-400 mt-3">
                Applied on {new Date(job.applied_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
              </p>
            )}
          </div>
          ...
          {/* Skills Match */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h2 className="text-sm font-semibold text-gray-900 mb-4">Skills Match</h2>

            {job.matched_skills?.length > 0 && (
              <div className="mb-4">
                <p className="text-xs font-medium text-emerald-600 mb-2">✓ Found in your resume</p>
                <div className="flex flex-wrap gap-2">
                  {job.matched_skills.map((skill, i) => (
                    <span key={i} className="px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-full text-xs font-medium">
                      {skill}
                    </span>
                  ))}
                </div>
            ...
            {job.missing_skills?.length > 0 && (
              <div>
                <p className="text-xs font-medium text-red-500 mb-2">✗ Missing from your resume</p>
                <div className="flex flex-wrap gap-2">
                  {job.missing_skills.map((skill, i) => {
                    const cat = CATEGORY_STYLES[skill.category] || CATEGORY_STYLES.other
                    return (
                      <span key={i} className={`px-3 py-1 ${cat.bg} ${cat.text} border ${cat.border} rounded-full text-xs font-medium`}>
                        {skill.name}
                      </span>
                    )
                  })}
            ...
            {!job.matched_skills?.length && !job.missing_skills?.length && (
              <p className="text-sm text-gray-400">No match data — upload your resume to see results.</p>
            )}
          </div>
          ...
          {job.keyword_gaps?.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <h2 className="text-sm font-semibold text-gray-900 mb-1">Keyword Gaps</h2>
              <p className="text-xs text-gray-400 mb-4">Add these keywords to your resume to improve your match score</p>
              <div className="space-y-3">
                {job.keyword_gaps.map((gap, i) => {
                  const p = PRIORITY_STYLES[gap.priority] || PRIORITY_STYLES.low
                  const cat = CATEGORY_STYLES[gap.category] || CATEGORY_STYLES.other
                  return (
                    <div key={i} className="flex items-start justify-between gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                      <div className="flex items-start gap-2 flex-wrap">
                        <span className="font-semibold text-gray-900 text-sm">{gap.keyword}</span>
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${cat.bg} ${cat.text} ${cat.border}`}>
                          {gap.category}
                        </span>
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${p.bg} ${p.text} ${p.border}`}>
                          {gap.priority} priority
                        </span>
                      </div>
                      <p className="text-xs text-gray-500 text-right max-w-[200px]">{gap.context}</p>
                    </div>
                  )
                })}
              </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'Experience', value: job.experience_min ? `${job.experience_min}${job.experience_max ? `—${job.experience_max}` : '+'} years` : null },
                { label: 'Education', value: job.education },
                { label: 'Added', value: new Date(job.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) },
              ].filter(item => item.value).map((item, i) => (
                <div key={i} className="flex items-start justify-between gap-2">
                  <span className="text-xs text-gray-400">{item.label}</span>
                  <span className="text-xs font-medium text-gray-700 capitalize text-right">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {job.preferred_skills?.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <h2 className="text-sm font-semibold text-gray-900 mb-3">Preferred Skills</h2>
              <div className="flex flex-wrap gap-2">
                {job.preferred_skills.map((skill, i) => (
                  <span key={i} className="px-2.5 py-1 bg-gray-50 text-gray-500 border border-gray-200 rounded-full text-xs font-medium">
                    {skill.name}
                  </span>
                ))}
              </div>
          ...
          <div className="bg-white rounded-2xl border border-red-100 shadow-sm p-6">
            ...
            {!showDeleteConfirm ? (
              ...
              <div className="space-y-2">
                <p className="text-xs text-gray-500">Are you sure? This cannot be undone.</p>
